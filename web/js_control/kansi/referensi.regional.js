$(function() {
    ...
    $("#btnBaru").click(function(e) {
        ...
        if ($("#btnBaru span").text() == "BARU") {
            ...
            $("#box-form-title").text("BUAT REGIONAL BARU");
            ...
            $("#box-form-title").text("FORM REGIONAL");
    ...
    $("#btnSimpan").click(function(e) {
        ...
        if ($("#btnSimpan").attr("disabled") != "disabled") {
            ...
            if (btnAction == "new") {
                var param = {
                    kode: $("#inputKode").val(),
                    wilayah: $("#inputJenis").val(),
                    token: TOKEN
                };

                $.ajax({
                    url: API_URL + "referensi/regional/add",
                    method: "POST",
                    dataType: "JSON",
                    ...
                    success: function(r) {
                        ...
                        if (!r.success)
                            ...
                            var newRow = '<tr>'+
                                ...
                                '<td><span id="text-'+r.result.kode+'">'+r.result.wilayah+'</span></td>'+
                ...
                $.ajax({
                    url: API_URL + "referensi/regional/edit/" + $("#inputKodeHidden").val(),
                    method: "POST",
                    dataType: "JSON",
                    data: JSON.stringify(param),
                    success: function(r) {
                        ...
                        if (!r.success)
                            ...
                            $("#text-"+old_code).text(r.result.wilayah);

                            $("#kode-"+old_code).attr("id", "kode-"+r.result.kode);
                            $("#text-"+old_code).attr("id", "text-"+r.result.wilayah);
                            
                            $("[data-kode="+old_code+"]").attr("data-jenis", r.result.wilayah);
    ...
    $("#btnSearch").click(function(e) {
        ...
        if ($("#keySearch").val().length > 0)
            window.location.href = WEB_URL+"kansi/referensi/regional?key="+$("#keySearch").val();
    });
...
function bindEdit() {
    $(".btnRefEdit").bind("click", function(e) {
        ...
        $("#box-form-title").text("EDIT REGIONAL");
        $("#inputKode").removeAttr("readonly");
    });
}
...
function bindDelete() {
    $(".btnRefDelete").bind("click", function(e) {
        ...
        $.confirm({
            title: "Konfirmasi",
            content: "<div style='text-align:center'>Apakah regional "+kode+" akan dihapus?</div>",
            buttons: {
                batal: function () {
                },
                hapus: function () {
                    ...
                    $.ajax({
                        url: API_URL + "referensi/regional/delete/" + kode,
                        method: "POST",
                        dataType: "JSON",
                        data: JSON.stringify(param),
                        success: function(r) {
