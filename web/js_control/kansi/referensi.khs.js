$(function() {
    ...
    $("#btnBaru").click(function(e) {
        ...
        if ($("#btnBaru span").text() == "BARU") {
            ...
            $("#btnSimpan, #inputTipe").removeAttr("disabled");
            ...
            $("#box-form-title").text("BUAT KHS BARU");

            var tipe = $("#inputTipe").val();
            if (tipe == "JASA")
                tipe = "REFERENSI-KHS-JASA";
            else if (tipe == "MATERIAL")
                tipe = "REFERENSI-KHS-MATERIAL";
    ...
    $("#btnSimpan").click(function(e) {
        ...
        if ($("#btnSimpan").attr("disabled") != "disabled") {
            ...
            if (btnAction == "new") {
                ...
                $.ajax({
                    url: API_URL + "referensi/khs/edit/" + $("#inputKode").val(),
                    method: "POST",
                    dataType: "JSON",
                    data: JSON.stringify(param),
                    success: function(r) {
    ...
    $("#btnSearch").click(function(e) {
        ...
        if ($("#keySearch").val().length > 0)
            window.location.href = WEB_URL+"kansi/referensi/khs?key="+$("#keySearch").val();
    });

    // control button clear search
    $("#btnClearSearch").click(function(e) {
        e.preventDefault();
        window.location.href = WEB_URL+"kansi/referensi/khs";
    });
...
function bindDelete() {
    $(".btnRefDelete").bind("click", function(e) {
        ...
        $.confirm({
            title: "Konfirmasi",
            content: "<div style='text-align:center'>Apakah khs "+kode+" akan dihapus?</div>",
            buttons: {
                batal: function () {
                },
                hapus: function () {
                    ...
                    $.ajax({
                        url: API_URL + "referensi/khs/delete/" + kode,
                        method: "POST",
                        dataType: "JSON",
                        data: JSON.stringify(param),
                        success: function(r) {
